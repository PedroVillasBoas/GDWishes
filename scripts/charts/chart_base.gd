...
extends Control
## Chart Foundation: margins, grid, entrance animation, and tooltip

const MARGIN := Rect2(56, 16, -72, -44)  # top-left and bottom-right offset

var anim := 0.0          # 0..1 | entrance animation progress
var _tooltip_text := ""
...
func draw_grid_and_axis(max_value: float, steps := 4) -> void:
	...
	for i in steps + 1:
		...
		draw_line(Vector2(r.position.x, y), Vector2(r.end.x, y), ThemeBuilder.BORDER, 1)
		var value := int(max_value * float(i) / steps)
		draw_string(font, Vector2(4, y + 4), _short_money(value),
			HORIZONTAL_ALIGNMENT_LEFT, 52, 11, ThemeBuilder.TEXT_DIM)

func _short_money(cents: int) -> String:   # 1.234.500 -> "12,3k"
...
func draw_chart_tooltip() -> void:
	...
	draw_rect(rect, ThemeBuilder.SURFACE_HI, true)
	draw_rect(rect, ThemeBuilder.BORDER, false, 1)
	draw_string(font, rect.position + Vector2(8, text_size.y + 1), _tooltip_text,
		HORIZONTAL_ALIGNMENT_LEFT, -1, 13, ThemeBuilder.TEXT)
