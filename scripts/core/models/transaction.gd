...
var notes: String = ""
## Set when this transaction was generated by confirming a recurring income/cost.
## Bookkeeping only — it is never shown, which is why it is a field of its own
## instead of a marker smuggled into `notes`.
...
static func from_dict(d: Dictionary) -> Transaction:
	...
	t.month = d.get("month", "")
	t.notes = d.get("notes", "")
	t.recurring_id = d.get("recurring_id", "")
	# Projects saved before recurring_id existed carry the marker inside notes.
	# Migrate it on load and clean the note, so old files stop showing that text.
	if t.recurring_id.is_empty() and t.notes.begins_with("[rec:"):
		var close := t.notes.find("]")
		if close > 5:
			t.recurring_id = t.notes.substr(5, close - 5)
			t.notes = t.notes.substr(close + 1).strip_edges()
			if t.notes == "gerado automaticamente":
				t.notes = ""
	return t
